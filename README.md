...
- 🔥 **JMeter scenario generator** (`js/jmeter.js`) — a questionnaire turns the spec into a ready-to-run **Apache JMeter 5.4.3** plan. Pick the shape (*spike arrest* burst, *quota* at a held rate, or a *ramp* that climbs until the 429s start), the requests (several endpoints in order or by traffic share, plus requests to other APIs by full URL), and the credential (fetched from the login endpoint it detects, pasted, or one per line from a CSV — with the token lifetime and whether it is refreshed once, on expiry or every iteration). It asks for the real host when the document has no `servers:`, and `429` counts as a pass so throttling reads as a result rather than as errors
...
├─ js/jmeter.js                  # JMeter .jmx wizard (spike arrest / quota tests)
