...
- 🔥 **JMeter test plans** (`js/jmeter.js`) — a four-question wizard turns the spec into a ready-to-run **Apache JMeter 5.4.3** plan: *spike arrest* (a burst released at one instant) or *quota / rate limit* (a steady rate held for a set time), against one endpoint, with the token fetched from the login endpoint the wizard finds in your document. `429` counts as a pass, so throttling shows up as a result rather than as errors
